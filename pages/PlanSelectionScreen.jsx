...
const PlanCard = ({ title, price, description, specialOffer, tag }) => {
  return (
    <View style={styles.planCard}>
      {tag && <View style={styles.tag}><Text style={styles.tagText}>{tag}</Text></View>}
...
const PlanSelectionScreen = () => {
  ...
  const onSwipe = ({ nativeEvent }) => {
    if (nativeEvent.translationX < -50 && nativeEvent.velocityX < -500 && nativeEvent.state === State.ACTIVE) {
...
>
  {plans.map((plan, index) => (
    <TouchableOpacity key={index} onPress={() => handleCardPress(plan)} style={styles.planCard}>
      {plan.tag && <View style={styles.tag}><Text style={styles.tagText}>{plan.tag}</Text></View>}
      <Text style={styles.planTitle}>{plan.title}</Text>
      <Text style={styles.price}>{plan.price}</Text>
      <Text style={styles.description}>{plan.description}</Text>
     <Text style={styles.billcycle}>{plan.billcycle}</Text>
     <Text style={styles.offertag}>{plan.offertag}</Text> 
      {plan.specialOffer && <Text style={styles.specialOffer}>{plan.specialOffer}</Text>}
